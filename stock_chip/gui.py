...
<body>
      ...
      <section class="panel">
        <div class="panel-head">
          <div>
            ...
            <div class="muted" id="sector-note">依平均區間漲跌排序，綠強紅弱。點族群卡看細分類與個股；統計樣本為已匯出的排行股票。</div>
          </div>
  ...
  <script>
    ...
    function renderSectorView() {
      ...
      const subs = aggregateGroups(list, r => r.sub_industry);
      ...
      if (state.sectorSub) list = list.filter(r => String(r.sub_industry || "") === state.sectorSub);
