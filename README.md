...
分頁「當沖候選」產生**盤前候選名單**。先講清楚它不是什麼：

> **這不是進出訊號。** 系統的資料全部是盤後的——沒有開盤價、沒有盤中價量、
> 分點還是 T+1。名單能把全市場 2,300 檔縮到 100 檔出頭，但何時進場、何時停損
> 只能靠看盤。當沖來回成本約 0.2–0.3%（證交稅減半 + 手續費），要先贏過這個
> 數字才回到原點。
...
| | 上市 | 上櫃 |
|---|---|---|
| 可交易性 | 當沖比率 ≥ 15% | 日均額 ≥ 5 億（**代理指標**） |
| 流動性 | 日均額 ≥ 3 億 | 同上 |

上櫃沒有公開的當沖統計——探測 workflow 把 TPEx openapi 的端點全列出來確認過，
命中的 12 個裡沒有任何一個是當沖成交統計。所以上櫃只能用成交額代理，門檻拉高
補償，並在 payload 的 `gate_basis` 與頁面上標示「代理指標」。日均額大但沒人當沖
的股票（中華電那類）盤中沒有當沖需要的流動性與波動，用日均額代理會放進來。

處置股一律排除：處置期間改人工撮合（約 2 分鐘一次），當沖實務上做不了。

**三種風格共用候選池，只有排序不同**，所以硬門檻只維護一份：

- **順勢追強**：前一日強勢族群成員、量比放大、法人同步買超；漲停仍有排隊買量加分
- **逆勢接刀**：昨日下跌但族群仍強、外資仍買；爆量下跌扣分（通常還有後續賣壓）
- **隔日沖賣壓**：主買分點為隔日沖券商

同一個訊號在不同風格是相反的：**隔日沖進駐在追強榜扣分（明天有賣壓），在賣壓榜
是必要條件**。這是三個榜分開做而非合成單一分數的核心理由。

資料來源（端點與欄位皆經探測 workflow 實測，2026-08）：

```bash
python3 -m stock_chip.daytrade
```

- TWSE `TWTB4U` 當沖標的成交量值——回 `tables` 結構，**沒有比率欄位**，比率用
  我們自己的 `daily_prices.volume` 去除，不依賴對方定義
- TWSE `openapi/announcement/punish` 與 TPEx `tpex_disposal_information` 處置股——
  處置期間格式不同（TWSE 全形「～」、TPEx 半形「~」），兩種都要能解析
- TPEx `tpex_ceil_non_trading` 漲停鎖死仍有排隊買量——盤後資料裡少數直接指向
  「明天開盤買盤」的訊號
